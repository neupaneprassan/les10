<template>
  <div class="layout2">
    <ClientOnly>
        ...
        >
          <div class="container p-4">
            <div>
              ...
              <h1
                v-if="data.subtitle"
                class="contenthead text-white opacity-80 pt-3 text-xl md:text-2xl lg:text-3xl font-bold pb-10"
              >
                {{ data.subtitle }}
              </h1>
            </div>
            ...
            <div>
              <p
                v-if="data.author"
                class="contentrender text-white opacity-80 text-xs font-bold"
              >
                {{ data.author }}
              </p>
      ...
      <!-- Main section -->
      <div
        class="container mx-auto p-4 animate-fade animate-once animate-delay-[500ms]"
        v-if="imageLoaded"
      >
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          ...
          <div>
            <h1
              class="contenthead text-3xl md:text-3xl lg:text-6xl lg:mb-3 font-bold"
            >
              {{ data.title }}
            </h1>
            <h1
              v-if="data.subtitle"
              class="contenthead opacity-80 text-xl md:text-2xl lg:text-3xl font-bold pb-10"
            >
              {{ data.subtitle }}
            </h1>
            <p
              class="contentrender text-lg md:text-2xl lg:text-xl pb-5 font-bold"
            >
              {{ data.description }}
            </p>
